...
import google.adk.a2a.utils.agent_card_builder as _acb
from google.adk.a2a.utils.agent_card_builder import AgentCardBuilder

# Ensure callable or non-string instructions don't cause TypeError in ADK's AgentCardBuilder
_orig_replace_pronouns = getattr(_acb, "_replace_pronouns", None)
if _orig_replace_pronouns:
    def _safe_replace_pronouns(text):
        if callable(text):
            text = text.__doc__ or ""
        if not isinstance(text, str):
            return ""
        return _orig_replace_pronouns(text)
    _acb._replace_pronouns = _safe_replace_pronouns

_orig_extract_examples = getattr(_acb, "_extract_examples_from_instruction", None)
if _orig_extract_examples:
    def _safe_extract_examples(instruction):
        if not isinstance(instruction, str):
            return None
        return _orig_extract_examples(instruction)
    _acb._extract_examples_from_instruction = _safe_extract_examples
